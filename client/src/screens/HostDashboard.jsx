  ...
  return (
    <div className="space-y-8">
      ...
      <section className="space-y-4">
        ...
        {matches.length === 0 && completedTournaments.length === 0 ? <Card><p className="text-sm text-[#A1A1A1]">No hosted matches found.</p></Card> : <div className="space-y-3">{matches.map((match) => <Card key={match._id}><div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="font-semibold text-white">{match.matchName}</h3><p className="text-xs text-[#A1A1A1]">{match.status} · {match.currentPlayers}/{match.maxPlayers} players · ₹{match.entryFee} entry</p></div><Button variant="secondary" size="sm" onClick={() => setSelectedMatchId(match._id)}>Manage Results</Button></div></Card>)}{completedTournaments.map((tournament) => <Card key={`completed-${tournament._id}`}><div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="font-semibold text-white">{tournament.name}</h3><p className="text-xs text-[#A1A1A1]">{tournament.game} · {tournament.format === 'single-match' ? 'Per Kill Tournament' : tournament.format}</p></div><span className="text-xs text-[#FFB066]">completed</span></div><div className="mt-3 grid gap-2 text-xs text-[#A1A1A1] sm:grid-cols-3"><span>Entry<br /><b className="text-white">₹{tournament.entryFee}</b></span><span>Paid Entries<br /><b className="text-white">{tournament.successfulEntries || 0}/{tournament.maxTeams}</b></span><span>Prize Pool<br /><b className="text-white">₹{Number(tournament.prizePool || 0).toLocaleString()}</b></span></div><div className="mt-3 rounded-lg border border-[#1F1F1F] bg-[#0B0B0B] p-3 text-xs text-[#A1A1A1]">{tournament.estimatedDate && <div><span className="text-[#A1A1A1]">Schedule:</span> <b className="text-white">{new Date(tournament.estimatedDate).toLocaleDateString()}</b></div>}<div className="mt-1"><span className="text-[#A1A1A1]">Status:</span> <b className="text-white">Completed</b></div></div><div className="mt-3"><button type="button" className="w-full rounded-lg border border-[#2B2B2B] bg-[#111111] px-3 py-2 text-sm font-semibold text-[#A1A1A1] opacity-70" disabled>Completed</button></div></Card>)}</div>}
      </section>
