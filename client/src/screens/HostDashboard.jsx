      ...
      <section className="space-y-4">
        ...
        {tournaments.length === 0 ? <Card><p className="text-sm text-[#A1A1A1]">No tournaments created yet.</p></Card> : (
          <div className="grid gap-4 md:grid-cols-2">
            {tournaments.map((tournament) => <Card key={tournament._id}><div className="flex items-start justify-between gap-3"><div><h3 className="font-semibold text-white">{tournament.name}</h3><p className="mt-1 text-xs text-[#A1A1A1]">{tournament.game} · {tournament.format === 'single-match' ? 'Per Kill Tournament' : tournament.format}</p></div><span className="text-xs text-[#FFB066]">{tournament.status}</span></div><div className={`mt-4 grid gap-2 text-xs text-[#A1A1A1] ${showPrizePool(tournament.format) ? 'grid-cols-3' : 'grid-cols-4'}`}><span>Entry<br /><b className="text-white">₹{tournament.entryFee}</b></span><span>Paid Entries<br /><b className="text-white">{tournament.successfulEntries || 0}/{tournament.maxTeams}</b></span>{showPrizePool(tournament.format) ? <span>Prize Pool<br /><b className="text-white">₹{Number(tournament.prizePool || 0).toLocaleString()}</b></span> : <span>Collected<br /><b className="text-white">₹{getCollectedAmount(tournament).toLocaleString()}</b></span>}{!showPrizePool(tournament.format) && <span>Per Kill<br /><b className="text-white">₹{Number(tournament.perKillReward || 0).toLocaleString()}</b></span>}</div><div className="mt-4 rounded-lg border border-[#1F1F1F] bg-[#0B0B0B] p-3 text-xs text-[#A1A1A1] space-y-1"><div><span className="text-[#A1A1A1]">Schedule:</span> <b className="text-white">{tournament.estimatedDate ? new Date(tournament.estimatedDate).toLocaleString() : 'No schedule yet'}</b></div>{tournament.estimatedTime && <div><span className="text-[#A1A1A1]">Time:</span> <b className="text-white">{tournament.estimatedTime}</b></div>}</div><div className="mt-4 rounded-lg border border-[#1F1F1F] bg-[#0B0B0B] p-3"><label className="block text-[10px] uppercase tracking-[0.2em] text-[#A1A1A1]">Host Message</label><textarea value={tournament.hostMessage || ''} onChange={(event) => { setTournaments((current) => current.map((item) => item._id === tournament._id ? { ...item, hostMessage: event.target.value } : item)); }} className="mt-2 w-full rounded-md border border-[#2B2B2B] bg-[#111111] px-3 py-2 text-sm text-white outline-none focus:border-[#FF6A00]" rows={3} placeholder="Add a message for players" /><div className="mt-3 flex justify-end"><Button variant="secondary" size="sm" onClick={() => handleMessageSave(tournament._id, tournament.hostMessage || '')}>Save Message</Button></div></div><div className="mt-4 flex gap-3"><Button variant="secondary" size="sm" className="flex-1" onClick={() => onNavigate?.('tournament-matches', tournament._id)}>Manage Matches & Results</Button><Button variant="secondary" size="sm" className="min-w-[78px]" onClick={() => handleDeleteTournament(tournament._id)}>Delete</Button></div></Card>)}
          </div>
        )}
      </section>
